...
private:
    ...
    bool CheckPathPlayerAndVirusCollision(std::vector<std::pair<int,int>> _viruspos);
    // 자르고 있지 않을 때 바이러스랑 충돌하는지
    bool CheckPlayerAndVirusCollision(std::vector<std::pair<int, int>> _viruspos);
    // 바이러스가 내가 먹은 땅 속에 있는지
    bool CheckVirusInClearedArea(Pixel* _pixel, std::vector<std::pair<int, int>> _viruspos);
